...
- FRAME (built, NOT wired): **VoxelCubeView** (`VoxelCubeView.swift:115–294` + the `voxel_raymarch` Metal kernel) — 64³ orthographic DDA raymarch (Amanatides–Woo), rest pose byte-identical to the 2D GIF hero, orbit reveals time-as-depth, scrub/auto-rotate/luma-floor/provenance filter. **It is explicitly SHELVED:** `PaletteRepresentation` (`PaletteGridView.swift:82–86`) has only `.structure/.grid/.cloud`; `.voxel3D` is commented out as "shelved until ready." The component compiles and is feature-complete; **only the Review-screen integration is missing.**

**Honesty facts.** Addressing ≠ dimensions: the 16²/4⁴/2⁸ branchings are *tree depth* (8 binary splits over 3 OKLab axes), never 8D. No embedding anywhere (no t-SNE/UMAP/PCA) — all views use true data coordinates. GRID Law #2: selection via opaque dark-step only, never alpha; per-slice opacity is forbidden.

**Unified phone navigation design.** A 4-mode representation selector — [structure] [grid] [cloud] [cube] — with two link mechanisms:
1. **Cross-view brushing** via a shared `brushedIndex` (plus a new multi-index brush): tap a colour in the Cloud → cube highlights all voxels using that index; tap a voxel region in the Cube → Cloud shows only those colours. The cube tap-pick + voxel→index extraction is the piece to build (`VoxelCubeView`).
...
VoxelCubeView is **the necessary GIFB-construction view** — the only surface that renders the (x,y,t) cube the global palette actually colours — so it must be wired into Review for the user to see GIFB at all. Smallest first truth-win: label the treemap split planes (e.g. `L@0.52`) at `PaletteTreeView.swift:72–75`.
...
2. **Wire VoxelCubeView into Review.** Add `.voxel3D` back to `PaletteRepresentation` and integrate the built component; add cross-view brushing scaffolding (`brushedIndex`) here. It is the only view of the (x,y,t) cube GIFB lives in.
