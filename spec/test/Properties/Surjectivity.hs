...
import SixFour.Spec.Indices (mkIndexTensor, mkCompleteVoxelVolume)
...
tests = testGroup "Surjectivity"
  ...
  , testProperty "CompleteVoxelVolume REJECTS a globally- but not per-frame-surjective tensor" $
      once $
        ...
        -- Global coverage is not enough — every frame must use all K colours.
        let v = [0,1,2,3, 0,1,0,1, 2,3,2,3]
            it = fromJust (mkIndexTensor @T @H @W @K v)
        in isNothing (mkCompleteVoxelVolume @T @H @W @K it)
  ]
