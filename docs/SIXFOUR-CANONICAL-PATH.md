...
| 2b-ii | **A/B capture → `btUpdate` + apply δ** — replace the `perturb()` stub (`AtlasState.swift:177`): on a pick, freeze the winner/loser 770-D embeddings into the `CMPE` record, fold θ via `btUpdate`, derive δ via `ThetaToDelta`, apply via `leafOverride`. The v0 unified core at **n=0** (literally Candidate 3). | `ab-perturb-stub` | head only | NEXT |
