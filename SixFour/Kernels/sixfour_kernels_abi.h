...
// THE MULTISCALE READS: exact i64 accumulations of a u16 10-bit world buffer
// (world[ch][subslice] layout per KernelsMultiScale.swift) at the three cadence
// scales — fast frame f, mid window k, slow window j — plus the fast→slow pool
// and the dead-time accounting. (Declared 2026-07-06: these five were exported
// but undeclared in the Zig era; the header is the ABI contract, so they are
// pinned here now.)
int64_t s4_ms_read_fast(const uint16_t *world, int32_t ch, int32_t f);
int64_t s4_ms_read_mid(const uint16_t *world, int32_t ch, int32_t k);
int64_t s4_ms_read_slow(const uint16_t *world, int32_t ch, int32_t j);
int64_t s4_ms_pool_fast_to_slow(const uint16_t *world, int32_t ch, int32_t j);
int64_t s4_ms_dead_time(const uint16_t *world, int32_t ch, int32_t j);
