"""native_kernels.py — ctypes binding to the SixFour owned Swift kernel core
(the trainer's synthetic-GIF DATA ENGINE). Formerly zig_native.py; the Zig core
was ported to Swift 2026-07-06 with the identical C ABI (@_cdecl), so the
ctypes signatures below are unchanged.
...
of libsixfour_kernels (trainer/lib/libsixfour_kernels.dylib) and exposes the
...
the iPhone runs (scripts/build-kernels-dylib.sh compiles the identical
SixFour/Kernels/*.swift). No train/deploy skew. The high-level `synth_sample()`
returns one labeled burst as numpy arrays.

Build the dylib first:  `scripts/build-kernels-dylib.sh`  (this module will also
auto-build it on import if it is missing and `swiftc` is on PATH).
"""
...
_BUILD_SCRIPT = _ROOT / "scripts" / "build-kernels-dylib.sh"
_DYLIB = _ROOT / "trainer" / "lib" / "libsixfour_kernels.dylib"


def _load() -> ctypes.CDLL:
    if not _DYLIB.exists():
        # Best-effort auto-build; surfaces a clear error if swiftc is absent.
        try:
            subprocess.run(["bash", str(_BUILD_SCRIPT)], cwd=_ROOT, check=True)
        except (FileNotFoundError, subprocess.CalledProcessError) as e:
            raise RuntimeError(
                f"{_DYLIB} not found and `{_BUILD_SCRIPT}` failed ({e}). "
                f"Run `{_BUILD_SCRIPT}` first."
            ) from e
