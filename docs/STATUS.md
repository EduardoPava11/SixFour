...
- **Deterministic Zig render core.** Per-stage kernels (widen → linear→OKLab → quantize
  ...
  Native header now **declares all 28** `s4_*` exports (25 shipped + 3 tooling-only:
...
| glrm-wired-but-unused | `Spec.GLRM` OLS kill-switch EXISTS + gated in spec; no Swift caller — AtlasTrainer must call it before value-net preference training | `spec/src/SixFour/Spec/GLRM.hs` | med | open |
| board-q16-unported | `Spec.BoardQ16` (`countsQ16`/`massQ16`, `lawCountsOrderIndependent`) is spec-only; live board MASS accumulation stays float/order-dependent (`Spec.AtlasBoard.histogram :: [OKLab] -> Double`) — float leak at the policy argmax | `spec/src/SixFour/Spec/BoardQ16.hs` (no Zig/Swift port) | high | open (blocker) |
