...
| **D16** the live grid/atom canon is `docs/SIXFOUR-TOTAL-PIXELATION.md` (4 pt atom); the old `docs/archive/SIXFOUR-DESIGN-LANGUAGE.md` is stale v2.0 (lineage only) | `docs/SIXFOUR-TOTAL-PIXELATION.md` (lineage: `docs/archive/SIXFOUR-DESIGN-LANGUAGE.md`) | med | **[INDEP]** | Keep the 4 pt atom canon in `docs/SIXFOUR-TOTAL-PIXELATION.md` (100×218, 256 pt preview, subPt retained, no blanket `[PLANNED]` on Spec.Lattice). STATUS:91 must NOT cite the archived design-language doc as evidence (circular: a stale doc "proving" a false status). Reconcile ScreenLattice/GlobalLattice/GridLayout as the single lattice authority. |
