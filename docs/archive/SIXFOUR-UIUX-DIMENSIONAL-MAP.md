...
- **Safe-area shift partially wired.** `safeTopPt`/`safeBottomPt` *are* emitted and used in `selfCheck()` clearance asserts (`LatticeContract.swift:34-35, 86`), but the runtime *whole-cell band shift* the doc describes is not applied by `GlobalLattice` (grid map §Gaps; partially corrects the "unused" claim — they are referenced, just not used to *shift*). → **RESOLVED 2026-06-05 (ADR-6, `SIXFOUR-UIUX-ARCHITECTURE-DECISIONS.md`):** safe-area is now **fixed margins** (top 11 / bottom 6 rows) owned by `ScreenLattice.swift`; content is pinned to absolute lattice rows below the Island. The dual-pitch + floating-VStack structural bug it caused is fixed there too (one `ScreenLattice`, one 6 pt pitch).
