...
-- σ₃₈₄ on the flat output: negate channels where sigmaDecoderMask is True.
sigmaDecoderList :: [Double] -> [Double]
sigmaDecoderList = zipWith (\b x -> if b then negate x else x) sigmaDecoderMask
...
tests = testGroup "GoldenForward (the concrete numeric forward — the golden-vector oracle)"
  ...
  [ testProperty "forward output is exactly 384-D (the SigmaPairTree genome)" $
      forAll genTokens $ \toks -> length (ftOutput (forward w toks)) == 384
  ...
  , testProperty "forward is σ-equivariant: output(σ_in tokens) ≈ σ₃₈₄(output tokens), tol 1e-9" $
      forAll genTokens $ \toks ->
        let a = ftOutput (forward w (map sigmaTok toks))
            b = sigmaDecoderList (ftOutput (forward w toks))
