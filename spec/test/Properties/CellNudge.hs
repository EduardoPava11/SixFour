...
tests = testGroup "CellNudge (corrected nudge: 9-at-cell over a 6-DOF voxel basis, gauge-explicit)"
  ...
  , testProperty "painting one channel in one cell is local (any grid, cell, pair, value)"
      lawPaintOnePairIsLocal
  , testProperty "REFUSAL: out-of-range (cell,pair) paint is a pinned no-op; negative value clamps"
