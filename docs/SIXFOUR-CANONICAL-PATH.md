...
| 1 | **Wire `s4_leaf_override`** (Zig kernel mirroring the proven-exact `LeafOverride.hs`, no tolerance), golden-gated Zig≡Swift≡Haskell + Swift caller — the σ-locked taste tint at `n=0`. | (new) | no | NEXT |
| 2 | **θ→δ map + A/B capture → `btUpdate`** — the v0 unified core, value-only, `n=0`. Closed-form linear `θ(770)→δ(384-DOF)`, golden-gated; this is *literally Candidate 3* at the **n=0 operating point**. Ships taste with ~zero new bit-exactness risk, proves the personalization channel before search exists. | `ab-perturb-stub` | head only | — |
