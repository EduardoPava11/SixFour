...
- FRAME (built + **WIRED**): **VoxelCubeView** (`VoxelCubeView.swift` + the `voxel_raymarch` Metal kernel) — 64³ orthographic DDA raymarch (Amanatides–Woo), rest pose byte-identical to the 2D GIF hero, orbit reveals time-as-depth, scrub/auto-rotate/luma-floor/provenance filter. **It is LIVE in Review** as the `.voxel3D` representation (`PaletteGridView.swift` `PaletteRepresentation.voxel3D`; selected and instantiated in `GIFReviewView.swift:76/123–128`, shown when `frameIndicesForVoxels != nil`). As of 2026-06-03 it also has the 2:1 dimetric isometric ruleset (az45/el30, 128-art-pixel quantization, auto-fit window), lock + iso-corner controls, flat-pose tap-pick brushing, and frame-isolation transparency (see `docs/SIXFOUR-VOXEL-CUBE.md` §8).

**Honesty facts.** Addressing ≠ dimensions: the 16²/4⁴/2⁸ branchings are *tree depth* (8 binary splits over 3 OKLab axes), never 8D. No embedding anywhere (no t-SNE/UMAP/PCA) — all views use true data coordinates. GRID Law #2: selection via opaque dark-step only, never alpha — with ONE sanctioned exception, the cube's frame-isolation *analysis mode* (owner-signed-off 2026-06-03), where non-focus slices take a ghost alpha so a single frame's palette can be studied (`docs/SIXFOUR-VOXEL-CUBE.md` §0.3 RULE-CUBE-ISOLATE).
...
2. **Wire VoxelCubeView into Review. — DONE (2026-06-03).** `.voxel3D` is live in `PaletteRepresentation`, the component is integrated in `GIFReviewView`, and the cross-view `brushedIndex`/`brushMode` scaffolding is threaded. Remaining cross-view work is the reverse Cloud-region → cube highlight (see §3 link mechanism 1).
