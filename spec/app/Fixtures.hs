...
import SixFour.Spec.ColorMomentum      (massOf, momentumOf, timeReverse)
...
main = do
  ...
  let opts      = parseArgs args
      ...
      nativeDir = fromMaybe "../SixFour/Kernels/LUTGoldens" (lookup "--native" opts)
  createDirectoryIfMissing True outDir
  createDirectoryIfMissing True nativeDir

  writeFile (outDir </> "color_golden.json") emitColorGolden
  writeFile (outDir </> "pbt_golden.json") emitPbtGolden
...
-- ---------------------------------------------------------------------------
-- PROPERTY-BASED cross-language cases (pbt_golden.json) — INSTITUTED 2026-07-06.
--
-- The fixed goldens above pin ONE case per kernel; these pin CASE FAMILIES:
-- a deterministic SplitMix64 stream (seed pinned below, byte-stable across
-- runs and GHC versions — deliberately NOT QuickCheck's Gen, whose stream is
-- not version-stable) drives random inputs through the SPEC ORACLES
-- (liftQuad, haarSplitTime, liftLevel, quantizeFrameQ16), and the Swift
-- kernels must reproduce every output byte-exactly
-- (SixFourTests/S4PropertyTests.swift replays this file). Property-based
-- testing with the Haskell spec as the oracle: the generator explores the
-- domain, the spec says what is correct, Swift agrees or fails loudly.
-- ---------------------------------------------------------------------------

-- | SplitMix64 step (the same constants the synth kernel pins): state -> (state', output).
pbtSm64 :: Word64 -> (Word64, Word64)
pbtSm64 s0 =
  let s  = s0 + 0x9E3779B97F4A7C15
      z1 = (s `xorW` (s `shiftR` 30)) * 0xBF58476D1CE4E5B9
      z2 = (z1 `xorW` (z1 `shiftR` 27)) * 0x94D049BB133111EB
  in (s, z2 `xorW` (z2 `shiftR` 31))
  where xorW = xor

-- | The infinite deterministic word stream for the pinned seed.
pbtStream :: [Word64]
pbtStream = go 0x5334375053425421  -- "S47PSBT!" — the instituted seed, never change
  where go s = let (s', z) = pbtSm64 s in z : go s'

-- | Map a word into [lo, hi] (mod bias accepted — this is a fixture, not statistics).
pbtRange :: Int -> Int -> Word64 -> Int
pbtRange lo hi w = lo + fromIntegral (w `mod` fromIntegral (hi - lo + 1 :: Int))

-- | Chop the stream into per-case input lists.
pbtTake :: Int -> [Word64] -> ([Word64], [Word64])
pbtTake = splitAt

emitPbtGolden :: String
emitPbtGolden =
  unlines
    [ "{"
    , "  \"_comment\": \"GENERATED by sixfour-spec / spec-fixtures — property-based case families, SplitMix64 seed pinned in Fixtures.hs. Oracles: RGBTLift.liftQuad, TemporalLoop.haarSplitTime, CubeLadder.liftLevel, QuantFixed.quantizeFrameQ16.\","
    , "  \"rgbt_quad\": [ " <> intercalate ", " rgbtCases <> " ],"
    , "  \"haar_time\": [ " <> intercalate ", " haarCases <> " ],"
    , "  \"cube_level\": [ " <> intercalate ", " cubeCases <> " ],"
    , "  \"quant\": [ " <> intercalate ", " quantCases <> " ],"
    , "  \"momentum\": [ " <> intercalate ", " momentumCases <> " ]"
    , "}"
    ]
  where
    ints xs    = "[" <> intercalate "," (map show xs) <> "]"
    triples ts = "[" <> intercalate ", " [ ints [a,b,c] | (a,b,c) <- ts ] <> "]"

    -- one flat stream, consumed section by section (order is part of the pin)
    (rgbtWs,  rest1) = pbtTake (256 * 4) pbtStream
    (haarWs,  rest2) = pbtTake (64 * 8 * 3) rest1
    (cubeWs,  rest3) = pbtTake (32 * 64) rest2
    (quantWs, rest4) = pbtTake (24 * 64 * 3) rest3
    (momWs,   _)     = pbtTake (64 * 8) rest4

    -- 256 random quads in ±32767 → lift (unlift is checked as roundtrip in Swift)
    rgbtCases =
      [ let [r, g, b, t] = map (pbtRange (-32767) 32767) (take 4 (drop (i * 4) rgbtWs))
            (lr, lg, lb, lt) = liftQuad (r, g, b, t)
        in "{ \"in\": " <> ints [r,g,b,t] <> ", \"out\": " <> ints [lr,lg,lb,lt] <> " }"
      | i <- [0 .. 255] ]

    -- 64 random 8-frame streams → temporal Haar split
    haarCases =
      [ let ws = take (8 * 3) (drop (i * 8 * 3) haarWs)
            frames = [ ( pbtRange (-32767) 32767 (ws !! (j*3))
                       , pbtRange (-32767) 32767 (ws !! (j*3+1))
                       , pbtRange (-32767) 32767 (ws !! (j*3+2)) ) | j <- [0 .. 7] ]
            (lo, hi) = haarSplitTime frames
        in "{ \"frames\": " <> triples frames
           <> ", \"low\": " <> triples lo
           <> ", \"high\": " <> triples hi <> " }"
      | i <- [0 .. 63] ]

    -- 32 random side-8 grids → one cube-ladder up-rung
    cubeCases =
      [ let grid = map (pbtRange (-32767) 32767) (take 64 (drop (i * 64) cubeWs))
            (coarse, dets) = liftLevel 8 grid
        in "{ \"grid\": " <> ints grid
           <> ", \"coarse\": " <> ints coarse
           <> ", \"details\": " <> triples dets <> " }"
      | i <- [0 .. 31] ]

    -- 24 random 8×8 frames (k=8, Lloyd iters cycling 0/1/4) → maximin+Lloyd
    quantCases =
      [ let ws = take (64 * 3) (drop (i * 64 * 3) quantWs)
            px = [ ( pbtRange 0 65535 (ws !! (j*3))
                   , pbtRange 0 65535 (ws !! (j*3+1))
                   , pbtRange 0 65535 (ws !! (j*3+2)) ) | j <- [0 .. 63] ]
            iters = [0, 1, 4] !! (i `mod` 3)
            (cs, asn) = quantizeFrameQ16 8 iters px
        in "{ \"iters\": " <> show iters
           <> ", \"pixels\": " <> triples px
           <> ", \"centroids\": " <> triples cs
           <> ", \"indices\": " <> ints asn <> " }"
    ...
    -- appended by the ColorMomentum institution (2026-07-06): 64 random blocks;
    -- oracle = Spec.ColorMomentum (mass reversal-even, momentum reversal-odd).
    momentumCases =
      [ let cells = map (fromIntegral . pbtRange (-32767) 32767) (take 8 (drop (i * 8) momWs))
        in "{ \"cells\": " <> ints cells
           <> ", \"mass\": " <> show (massOf cells)
           <> ", \"momentum\": " <> show (momentumOf cells)
           <> ", \"reversed\": " <> ints (timeReverse cells) <> " }"
      | i <- [0 .. 63] ]
